# Beech tree health grade prediction
...
XXX scripts can be found in the `scripts` subfolder: 

1. `downloadNDVIdiff.py`
2. `subsampleLAS.py`
3. `funPeaks_batch.m`
3. `images_processing/scripts/calculate_ndvi.py`
4. `images_processing/scripts/stats_per_tree.py`
... 
4. `FHI_catalog.R`
5. `mergeData_inpoly.R`
6. `RF.R`

which can be run one after the other following this very order.

Note

Different configuration files are to be used for all the scripts
...
* **tiles**, or - more explicitly - "geographical map tiles": cf. [this link](https://wiki.openstreetmap.org/wiki/Tiles). 
...
Yearly NDVI differences download from waldmonitoring.ch. Run the script `/scritps/downloadNDVIdiff.py` in the Python environment. 
...
LAS point cloud segmentation is performed using the Digital Forestry Toolbox on Matlab/Octave:
* Go to Matthew Parkan GitHub: https://mparkan.github.io/Digital-Forestry-Toolbox/
...
Structural descriptors are computed via RStudio mainly after the article from P. Meng et al (2022), DOI: 10.1080/17538947.2022.2059114:
...
* Addition of extra trees after RF prediction control on field (July 2023)
* Delivery of a point SHP file (coordinates + attributes) from the foresters
* Coordinates correction with the help of the LiDAR point cloud (viewed in potree) and of the pictures taken on field.
* Removal of other species than beech trees
* Removal of candles (very late stage of dead trees, only trunk left)
* Removal of a few trees that are un-sharp the true orthophoto
...
* Conversion of 5 classes (healthy, a bit unhealthy, middle unhealthy, very unhealthy, dead) to 3 classes (healthy, unhealthy, dead)
