...
if ~only_SHP_points_output_FLAG
  ...
  ASCwrite(strcat(PathOut, '_dtm.asc'), ...
  ...
  ASCwrite(strcat(PathOut, '_dsm.asc'), ...
  ...
  ASCwrite(strcat(PathOut, '_dhm.asc'), ...
...
[peaks_crh, ~] = canopyPeaks(models.height.values, ...
    refmat, ...
    'minTreeHeight', 3, ...
    %'minHeightDifference', 0.1, ...
    'searchRadius', (3.09632 + 0.00895 * h^2)/2, ...
    'fig', false, ...
    'verbose', true);
...
if ~only_SHP_points_output_FLAG
  ...
  fid = fopen(strcat(PathOut, '_seg_metrics.csv'), 'w+'); % open file
...
if OCTAVE_FLAG
    shapewrite(S1,strcat(PathOut, '_peaks.shp'));%
else
    shapewrite(S1, strcat(PathOut, '_peaks.shp'));
end
clear S1
...
if ~only_SHP_points_output_FLAG
  ...
  % function from the offical Matlab mapping toolbox instead.
  if OCTAVE_FLAG
    shapewriteOctave(S2, strcat(PathOut, '_seg_polygons.shp'));
  else
    shapewrite(S2, strcat(PathOut, '_seg_polygons.shp'));
  end
clear S2
...
if ~only_SHP_points_output_FLAG
  ...
  r2 = LASread(strcat(PathOut, '_ws_seg.las'));
